define(["require", "exports", 'knockout', 'jquery', 'moment'], function (require, exports, ko, $, moment) {
    var DatePicker;
    (function (DatePicker) {
        ko.bindingHandlers['datepicker'] = {
            init: function (element, valueAccessor, allBindingsAccessor) {
                //initialize datepicker with some optional options
                var options = allBindingsAccessor().datepickerOptions || {}, format = !element.getAttribute('datepicker-format') ? 'mm-dd-yyyy' : element.getAttribute('datepicker-format');
                $(element)['datepicker']({
                    format: format
                });
                ...
                ko.utils.registerEventHandler(element, 'change', function (event) {
                    ...
                    var finalDateValue = event.date ? moment(event.date).format('YYYY-MM-DD') : (textValue != "" && moment(textValue, 'MM-DD-YYYY').isValid() ? moment(textValue, 'MM-DD-YYYY').format('YYYY-MM-DD') : '');
                    if (finalDateValue) {
            ...
            update: function (element, valueAccessor) {
                ...
                if (widget) {
                    if (ko.utils.unwrapObservable(valueAccessor() != null) && ko.utils.unwrapObservable(valueAccessor() != ""))
                        widget.date = new Date(moment(ko.utils.unwrapObservable(valueAccessor())).format());
                    if (valueAccessor() != "") {
